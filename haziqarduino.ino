...
String scripttext = "const timer = document.querySelector(\".timer\"); let timerTime = " + useDuration +" * 60 * 1000; setInterval(() => {timerTime--; timer.innerHTML = `${Math.floor(times / 60000) % 60}:${Math.floor(times / 1000) % 60}`}, timerTime)";
String script = "";
...
void loop() {
  ...
  client.println("  <div class='status-box timer'>" + timertext + "</div>");
  client.println("<script>" + script + "</script>");
